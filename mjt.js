/**
 * Monitus JavaScript Tools v7
 * Copyright (c) 2011-2012 Monitus, LLC. All rights reserved.
...
if(typeof(GMT) == "undefined") {
	var GMT = function() {
		...
		return {
			...
			reloadOnNav: function(pFunctionCall) {
				var vCall = pFunctionCall + "();";
				setInterval(vCall, 100);
				eval(vCall);
			},
			string2number: function(pValue) {
				if(typeof(pValue) != "string") return pValue;
				return parseFloat(pValue.replace(/[^\.0-9\-]/ig, ''));
			},
			
			ga: {
				currentSection: function() {
					var url = decodeURIComponent(document.location);
					var vTokens = url.match(/explorer\-sectionControl\.sectionId\=(ecommerce|goal_set\d+|site_usage)/i);
					if(vTokens) return vTokens[1];
					return "summary";
				},
				currentView: function() {
					var url = decodeURIComponent(document.location);
					var vTokens = url.match(/goalOption\=\d/i);
					if(vTokens) return "goal";
					return "ecommerce";
				},
				stats: {
					ui: function() {
						if(GMT.$("#monitus-z-test").length > 0) return;
						var vSection = GMT.ga.currentSection();
						if(!vSection.match(/(ecommerce|goal_set\d+|summary)/i)) return;
						var vButtonTemplate = GMT.$(".ACTION-plotKeys");
						var vButton = vButtonTemplate.clone().attr("type", "button").attr("id", "monitus-z-test").css({"min-width": "65px"}).html("Z-test").val("Z-test");
						vButton.removeClass("ACTION-plotKeys").addClass("ACTION-ztest");
						vButton.removeClass("ID-plotKeys").addClass("ID-ztest");
						vButtonTemplate.after(vButton);
						vButton.click(GMT.ga.stats.zTest);
						if(GMT.$("#monitus-z-test-result").length <= 0) {
							var vField = GMT.$('<div id="monitus-z-test-result"></div>');
							vField.insertBefore(GMT.$("#ID-explorer-segmentExplorer"));
							vField.css("font-weight", "bold");
							vField.css("padding", "6px");
						}
						
						var vCheckboxes = GMT.$("#ID-explorer-table-dataTable .ID-checkbox").click(function() {
							var selection = GMT.$("#ID-explorer-table-dataTable .ID-checkbox:checked");
							if(selection.length != 2) {
								vButton.attr("disabled", "disabled");
								GMT.ga.stats.zTestClear();
							} else vButton.removeAttr("disabled");
						});
					},
					zTest: function(pEvent) {
						pEvent.preventDefault();
						pEvent.stopPropagation();
						var vCheckboxes = GMT.$("#ID-explorer-table-dataTable .ID-checkbox:checked");
						if(!vCheckboxes || (vCheckboxes.length != 2)) {
							alert("Please select TWO rows to compare, using the checkboxes on the left.");
							if(vCheckboxes) vCheckboxes.parent().css("background-color", "#ffcc66");
						} else {
							var vSection = GMT.ga.currentSection();
							var vView = GMT.ga.currentView();
							var vNote = "";
							var vConvRateColID = "ID-metric-data-8";
							var vConvRateName = "ecommerce";
							if(vSection.match(/goal_set\d+/i)) {
								vConvRateColID = "ID-metric-data-1";
								vConvRateName = "goal";
								vNote = "Please note that the overall goal conversion rate is used, which is a sum of all goals. Use appropriately.";
							} else if(vSection.match(/ecommerce/i)) vConvRateColID = "ID-metric-data-4";
							else if(vView == "goal") vConvRateColID = "ID-metric-data-6";
							
							var vVisits = vCheckboxes.parent().siblings(".ID-metric-data-0").get();
							var vConvRates = vCheckboxes.parent().siblings("." + vConvRateColID).get();
							if(vVisits && vConvRates && (vVisits.length == 2) && (vConvRates.length == 2)) {
								vVisits[0] = GMT.string2number(vVisits[0].innerHTML);
								vVisits[1] = GMT.string2number(vVisits[1].innerHTML);
								vConvRates[0] = GMT.string2number(vConvRates[0].innerHTML) / 100;
								vConvRates[1] = GMT.string2number(vConvRates[1].innerHTML) / 100;
								
								if(((vVisits[0] * vConvRates[0]) <= 5.0) || ((vVisits[1] * vConvRates[1]) <= 5.0)) alert("Unfortunately that sample size is too small to be significant. Please pick rows with higher values.");
								else {
									var vDiff = Math.abs(vConvRates[0] - vConvRates[1]);
									var vStdDeviation = Math.sqrt((vConvRates[0] * (1 - vConvRates[0]) / vVisits[0]) + (vConvRates[1] * (1 - vConvRates[1]) / vVisits[1]));
									var vZScore = vDiff / vStdDeviation;
									var vResult = 1 - (2 * (1 - jstat.pnorm(vZScore)));
									
									vMessage = Math.round(vResult * 100) + "% Confidence that " + vConvRateName + " conversion rates are different";
									if(vResult > 0.95) {
										vColor = "green";
									} else {
										vColor = "red";
									}
									var vField = GMT.$("#monitus-z-test-result");
									if(vField) {
										vMessage += ' <a href="#" class="note">[&nbsp;?&nbsp;]</a>';
										vField.html(vMessage);
										vField.css("color", vColor);
										vField.children("a.note").first().toggle(vNote && (vNote != "")).data("note", vNote).click(function(pEvent) {
											pEvent.preventDefault();
											pEvent.stopPropagation();
											alert(vField.children("a.note").data("note"));
										});
									} else alert(vMessage);
								}
							}
					...
					zTestClear: function() {
						GMT.$("#monitus-z-test-result").html("");
					}
				}
			}
		};
	...
	vScript.src = "//ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js";
	vScript.onload = vScript.onreadystatechange = function(){
		try {
			GMT.$ = jQuery.noConflict();
			GMT.reloadOnNav("GMT.ga.stats.ui");
		} catch(e) {
 ...
 * This document is licensed as free software under the terms of the
 * MIT License: http://www.opensource.org/licenses/mit-license.php */
function jstat(){};(function(){var initializing=false,fnTest=/xyz/.test(function(){xyz;})?/\b_super\b/:/.*/;this.Class=function(){};Class.extend=function(prop){var _super=this.prototype;initializing=true;var prototype=new this();initializing=false;for(var name in prop){prototype[name]=typeof prop[name]=="function"&&typeof _super[name]=="function"&&fnTest.test(prop[name])?(function(name,fn){return function(){var tmp=this._super;this._super=_super[name];var ret=fn.apply(this,arguments);this._super=tmp;return ret;};})(name,prop[name]):prop[name];}
...
var now=new Date();var hash=now.getMilliseconds()*now.getMinutes()+now.getSeconds();GMT.$('body').append('<div title="'+title+'" style="display: none;" id="'+hash+'"><div id="graph-'+hash+'" style="width:95%; height: 95%"></div></div>');GMT.$('#'+hash).dialog({modal:false,width:475,height:475,resizable:true,resize:function(){$.plot(GMT.$('#graph-'+hash),[series],flotOpt);},open:function(event,ui){var id='#graph-'+hash;$.plot(GMT.$('#graph-'+hash),[series],flotOpt);}})}
...
function showTooltip(x,y,contents,color){GMT.$('<div id="jstat_tooltip">'+contents+'</div>').css({position:'absolute',display:'none',top:y+15,'font-size':'small',left:x+5,border:'1px solid '+color[1],color:color[2],padding:'5px','background-color':color[0],opacity:0.80}).appendTo("body").show();}
var previousPoint=null;GMT.$(this._container).bind("plothover",function(event,pos,item){GMT.$("#x").text(pos.x.toFixed(2));GMT.$("#y").text(pos.y.toFixed(2));if(item){if(previousPoint!=item.datapoint){previousPoint=item.datapoint;GMT.$("#jstat_tooltip").remove();var x=jstat.toSigFig(item.datapoint[0],2),y=jstat.toSigFig(item.datapoint[1],2);var text=null;var color=item.series.color;if(item.series.label=='PDF'){text="P("+x+") = "+y;color=["#fee","#fdd","#C05F5F"];}else{text="F("+x+") = "+y;color=["#eef","#ddf","#4A4AC0"];}
showTooltip(item.pageX,item.pageY,text,color);}}
else{GMT.$("#jstat_tooltip").remove();previousPoint=null;}});GMT.$(this._container).bind("mouseleave",function(){if(GMT.$('#jstat_tooltip').is(':visible')){GMT.$('#jstat_tooltip').remove();previousPoint=null;}});}else{if(this._options.grid==null){this._options.grid={hoverable:false}}else{this._options.grid.hoverable=false}
GMT.$(this._container).unbind("plothover");}
